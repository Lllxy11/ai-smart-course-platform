  ...
  <div class="exam-management">
    <div class="page-header">
      ...
      <div class="header-actions">
        ...
        <el-button type="success" @click="exportExams">
    ...
    <el-card class="table-card">
      ...
      >
        ...
        <el-table-column prop="teacher" label="授课教师" width="120">
          <template #default="{ row }">
            {{ row.teacher?.realName || row.teacher?.username }}
          </template>
        </el-table-column>
    ...
    >
      ...
      >
        ...
        <el-form-item label="授课教师" prop="teacherId">
          <el-select v-model="examForm.teacherId" placeholder="选择教师" filterable style="width: 100%">
            <el-option
              v-for="teacher in teachers"
              :key="teacher.id"
              :label="teacher.realName || teacher.username"
        ...
        <el-row :gutter="20">
          <el-col :span="8">
            <el-form-item label="考试状态" prop="status">
              <el-select v-model="examForm.status" placeholder="选择状态" style="width: 100%">
                <el-option label="草稿" value="DRAFT" />
                <el-option label="已发布" value="PUBLISHED" />
                <el-option label="已结束" value="ENDED" />
              </el-select>
      ...
      <div v-if="selectedExam" class="exam-detail">
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="授课教师">
            {{ selectedExam.teacher?.realName || selectedExam.teacher?.username }}
          </el-descriptions-item>
          ...
          <el-descriptions-item label="创建时间">
            {{ formatTime(selectedExam.createdAt) }}
          </el-descriptions-item>
...
const handleSaveExam = async () => {
  ...
  try {
    ...
    if (isEditing.value) {
      await updateExam(examForm.id, examForm)
...
const viewExam = async (exam) => {
  ...
  try {
    // 加载考试详细统计数据
    const response = await getExamStatistics(exam.id)
    examDetailStats.value = response.data || {
...
const publishExam = async (exam) => {
  try {
    await updateExam(exam.id, { ...exam, status: 'PUBLISHED' })
...
const endExam = async (exam) => {
  try {
    await updateExam(exam.id, { ...exam, status: 'ENDED' })
...
const batchPublish = async () => {
  if (selectedExams.value.length === 0) {
    ElMessage.warning('请选择要发布的考试')
    return
  }
  
  try {
    const promises = selectedExams.value.map(exam => 
      updateExam(exam.id, { ...exam, status: 'PUBLISHED' })
    )
...
const batchArchive = async () => {
  if (selectedExams.value.length === 0) {
    ElMessage.warning('请选择要归档的考试')
    return
  }
  
  try {
    const promises = selectedExams.value.map(exam => 
      updateExam(exam.id, { ...exam, status: 'ENDED' })
    )
    await Promise.all(promises)
...
const batchDelete = async () => {
  ...
  try {
    await ElMessageBox.confirm(
      `确定要删除选中的 ${selectedExams.value.length} 个考试吗？此操作不可恢复！`,
      '批量删除考试',
      {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
    ...
    const promises = selectedExams.value.map(exam => deleteExam(exam.id))
    await Promise.all(promises)
